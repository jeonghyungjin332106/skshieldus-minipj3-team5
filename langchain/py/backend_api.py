...
def get_agent_executor(user_id: str, temperature: float = 0.0):
    ...
    # 에이전트 프롬프트 설정
    # MessagesPlaceholder를 사용하여 대화 기록을 동적으로 삽입합니다.
...
**현재 사용 중인 이력서 파일 경로:** {effective_file_path}

**사용 가능한 기능:**
1.  **직무 및 역량 추천**: 이력서 파일을 기반으로 추천 직무와 필요 역량을 알려줍니다.
    * 예시 요청: "이력서 파일로 저한테 맞는 직무랑 필요한 역량 추천해주세요."
    * 툴 호출 예시: `recommend_job_and_skills_tool('{{"file_path": "{effective_file_path}", "temperature": 0.5, "num_recommendations": 3}}')`
2.  **면접 예상 질문 생성**: 이력서 파일을 기반으로 특정 회사, 면접 유형, 희망 직무에 맞는 면접 질문을 생성합니다.
    * **중요 예시**: "이력서 파일('{effective_file_path}')로 ABC 회사 기술 면접 질문 5개 만들어주세요."
    * **다른 예시**: "이력서 파일('{effective_file_path}')로 마케터 직무의 면접 질문을 3개 만들어주세요."
    * **DOCX 파일 예시**: "이력서 파일('{effective_file_path}')로 스타트업 인성 면접 질문 4개 만들어주세요."
    * **면접 유형 지정**: 요청에 '기술 면접', '인성 면접', '행동 기반 질문' 등의 키워드를 포함하여 면접 유형을 지정할 수 있습니다. (기본값은 'general'입니다.)
    * 툴 호출 예시: `generate_interview_questions_tool('{{"file_path": "{effective_file_path}", "company_name": "ABC 회사", "interview_type": "technical", "desired_job_role": "백엔드 개발자"}}')`
3.  **면접 답변 피드백 및 개선된 답변 생성**: 면접 질문과 사용자의 답변을 받아 피드백을 제공하고, 개선된 답변을 제안합니다.
    * **피드백 요청 예시**: "질문: 자기소개 해주세요 답변: 저는 성실하고 꼼꼼합니다."
    * **또는**: "면접 질문 '자기소개 해주세요'에 대해 '저는 성실하고 꼼꼼합니다.'라고 답했는데 피드백과 개선된 답변을 알려주세요."
    * 툴 호출 예시: `get_interview_feedback_and_improved_answer_tool('{{"file_path": "{effective_file_path}", "original_question": "자기소개 해주세요", "user_answer": "안녕하세요. 저는...", "company_name": "ABC 마케팅", "job_role": "디지털 마케터"}}')`
...
async def chat_with_ai(request_data: ChatRequest):
    ...
    try:
        ...
        # file_path는 시스템 프롬프트에 이미 포함되어 있으므로, 여기서는 input과 chat_history만 전달합니다.
        result = await agent_executor.invoke({
            "input": user_message,
            "chat_history": chat_history,
            # "current_file_path": file_path # 이미 get_agent_executor에서 사용됨
        })
