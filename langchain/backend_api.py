...
user_data_store = {} # {user_id: {'file_path': ..., 'chunk_size': ..., 'chunk_overlap': ..., 'chat_history': []}}
...
def _load_document_to_vector_store(file_path: str, chunk_size: int = 1000, chunk_overlap: int = 100):
    ...
    지원 형식: PDF, DOCX, TXT
    """
    print(f"[_load_document_to_vector_store] 호출됨. 파일 경로: {file_path}")
    if not os.path.exists(file_path):
        print(f"[_load_document_to_vector_store] 오류: 파일 '{file_path}'을(를) 찾을 수 없습니다.")
...
def recommend_job_and_skills_tool(input_json: str) -> str:
    ...
    """
    print(f"[recommend_job_and_skills_tool] 호출됨. 입력: {input_json}")
    try:
        ...
        if not file_path:
            print("[recommend_job_and_skills_tool] 오류: file_path가 제공되지 않았습니다.")
            return json.dumps({"error": "file_path가 제공되지 않았습니다."}, ensure_ascii=False)
        if not os.path.exists(file_path):
            print(f"[recommend_job_and_skills_tool] 오류: 파일 '{file_path}'을(를) 찾을 수 없습니다.")
            return json.dumps({"error": f"파일을 찾을 수 없습니다: {file_path}. 파일을 먼저 업로드해주세요."}, ensure_ascii=False)

        # _load_document_to_vector_store 함수는 툴이 호출될 때마다 벡터 저장소를 생성합니다.
...
답변:"""
        ...
        print(f"[recommend_job_and_skills_tool] 성공적으로 결과 반환.")
        return json.dumps(parsed_response, ensure_ascii=False, indent=2)

    except Exception as e:
        print(f"[recommend_job_and_skills_tool] 오류 발생: {str(e)}")
...
def generate_interview_questions_tool(input_json: str) -> str:
    ...
    """
    print(f"[generate_interview_questions_tool] 호출됨. 입력: {input_json}")
    try:
        params = json.loads(input_json)
        file_path = params.get('file_path')
        ...
        if not file_path:
            print("[generate_interview_questions_tool] 오류: file_path가 제공되지 않았습니다.")
            return json.dumps({"error": "file_path가 제공되지 않았습니다."}, ensure_ascii=False)
        if not os.path.exists(file_path):
            print(f"[generate_interview_questions_tool] 오류: 파일 '{file_path}'을(를) 찾을 수 없습니다.")
            return json.dumps({"error": f"파일을 찾을 수 없습니다: {file_path}. 파일을 먼저 업로드해주세요."}, ensure_ascii=False)
        ...
        company_instruction = f"면접 볼 회사가 '{company_name}'이라면, 해당 회사의 비전, 제품, 문화와 연관된 질문을 2~3개 포함해주세요." if company_name else ""
        job_role_instruction = ""
        if desired_job_role:
            job_role_instruction = f"지원자가 희망하는 직무는 '{desired_job_role}'입니다. 이 직무에 필요한 역량, 경험, 그리고 직무 관련 시나리오에 대한 질문을 2~3개 포함해주세요."
...
답변:"""
        ...
        parsed_response = parser.parse(response['answer'])
        print(f"[generate_interview_questions_tool] 성공적으로 결과 반환.")
        return json.dumps(parsed_response, ensure_ascii=False, indent=2)

    except Exception as e:
        print(f"[generate_interview_questions_tool] 오류 발생: {str(e)}")
...
def get_interview_feedback_and_improved_answer_tool(input_json: str) -> str:
    ...
    """
    print(f"[get_interview_feedback_and_improved_answer_tool] 호출됨. 입력: {input_json}")
    try:
        params = json.loads(input_json)
        file_path = params.get('file_path')
        ...
        if not all([file_path, original_question, user_answer]):
            print("[get_interview_feedback_and_improved_answer_tool] 오류: file_path, original_question, user_answer는 필수입니다.")
            return json.dumps({"error": "file_path, original_question, user_answer는 필수입니다."}, ensure_ascii=False)
        if not os.path.exists(file_path):
            print(f"[get_interview_feedback_and_improved_answer_tool] 오류: 파일 '{file_path}'을(를) 찾을 수 없습니다.")
            return json.dumps({"error": f"파일을 찾을 수 없습니다: {file_path}. 파일을 먼저 업로드해주세요."}, ensure_ascii=False)
...
개선된 답변:"""
        ...
        print(f"[get_interview_feedback_and_improved_answer_tool] 성공적으로 결과 반환.")
        return json.dumps(full_result, ensure_ascii=False, indent=2)

    except Exception as e:
        print(f"[get_interview_feedback_and_improved_answer_tool] 오류 발생: {str(e)}")
...
def get_agent_executor(user_id: str, temperature: float = 0.0):
    ...
    """
    print(f"[get_agent_executor] 호출됨. User ID: {user_id}")
    # 사용자 데이터 스토어에서 해당 user_id의 데이터 가져오기
    user_data = user_data_store.get(user_id, {})
    
    # 현재 사용 중인 파일 경로. 이 값은 시스템 프롬프트에 직접 삽입됩니다.
    effective_file_path = user_data.get('file_path', '없음') 
    print(f"[get_agent_executor] Effective file path for agent: {effective_file_path}")
    ...
    llm = ChatOpenAI(model="gpt-3.5-turbo", temperature=float(temperature), api_key=OPENAI_API_KEY)
...
**현재 사용자에게 업로드된 이력서 파일 경로:** `{current_resume_path}`
**중요:** 이 경로(`{current_resume_path}`)를 **모든 툴 호출의 'file_path' 인자 값**으로 사용해야 합니다.
만약 사용자가 대화 중 다른 파일 이름을 명시적으로 언급하면, 그 파일을 'file_path' 인자로 사용하세요.
하지만, 사용자가 파일 이름을 명시하지 않았다면, 항상 **`{current_resume_path}`**를 사용해야 합니다.

**사용 가능한 기능 (툴 호출 시 'file_path' 인자는 항상 현재 업로드된 파일 경로를 사용해야 합니다):**
1.  **직무 및 역량 추천**: 이력서 파일을 기반으로 추천 직무와 필요 역량을 알려줍니다.
    * 예시 요청: "이력서 파일로 저한테 맞는 직무랑 필요한 역량 추천해주세요."
    * 툴 호출 형식: `recommend_job_and_skills_tool(input_json='{"file_path": "_CURRENT_FILE_PATH_", "temperature": 0.5, "num_recommendations": 3}')`
2.  **면접 예상 질문 생성**: 이력서 파일을 기반으로 특정 회사, 면접 유형, 희망 직무에 맞는 면접 질문을 생성합니다.
    * 예시 요청: "이력서 파일로 ABC 회사 기술 면접 질문 5개 만들어주세요."
    * 툴 호출 형식: `generate_interview_questions_tool(input_json='{"file_path": "_CURRENT_FILE_PATH_", "company_name": "ABC 회사", "interview_type": "technical", "desired_job_role": "백엔드 개발자"}')`
3.  **면접 답변 피드백 및 개선된 답변 생성**: 면접 질문과 사용자의 답변을 받아 피드백을 제공하고, 개선된 답변을 제안합니다.
    * 예시 요청: "질문: 자기소개 해주세요 답변: 저는 성실하고 꼼꼼합니다."
    * 툴 호출 형식: `get_interview_feedback_and_improved_answer_tool(input_json='{"file_path": "_CURRENT_FILE_PATH_", "original_question": "자기소개 부탁드립니다.", "user_answer": "안녕하세요. 저는...", "company_name": "ABC 마케팅", "job_role": "디지털 마케터"}')`

**중요:**
-   모든 툴은 'file_path'를 필수로 요구합니다. 만약 대화 기록에 유효한 파일 경로가 없거나, 현재 요청에 명시되지 않았고 이전에 설정된 파일 경로가 없다면, 사용자에게 파일 경로를 먼저 물어보세요.
-   'file_path'는 './uploads/예시 파일 (마케터).pdf', './uploads/예시 파일 (개발자).txt' 또는 './uploads/다른 파일.docx'와 같은 형식이어야 합니다.
-   사용자가 답변 피드백을 요청할 경우, 'original_question'과 'user_answer'를 반드시 받아야 합니다. 에이전트는 사용자의 입력에서 이 두 정보를 정확히 파싱하여 `get_interview_feedback_and_improved_answer_tool`에 JSON 형태로 전달해야 합니다.
-   툴 실행 결과는 사용자에게 친절하고 이해하기 쉽게 설명해주세요.
"""
    # .format()을 사용하여 effective_file_path를 삽입합니다.
    formatted_system_message = system_message_template.format(current_resume_path=effective_file_path)
    ...
    print(f"[get_agent_executor] Agent prompt created.")

    agent = create_tool_calling_agent(llm, tools, prompt)
    agent_executor = AgentExecutor(agent=agent, tools=tools, verbose=True)
    print(f"[get_agent_executor] AgentExecutor created.")
...
async def upload_resume(
    ...
    지원 형식: PDF, DOCX, TXT
    """
    print(f"[upload_resume] 파일 업로드 요청 수신. User ID: {userId}, 파일명: {file.filename}")
    if file.filename == '':
    ...
    try:
        ...
        print(f"[upload_resume] 파일 '{file.filename}' 저장 완료: {file_path}")
    except Exception as e:
        print(f"[upload_resume] 파일 저장 중 오류 발생: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"파일 저장 중 오류 발생: {str(e)}")

    try:
        # 사용자 데이터 스토어에 파일 경로 및 청크 설정 저장
        if userId not in user_data_store:
            user_data_store[userId] = {'chat_history': []}
        
        user_data_store[userId]['file_path'] = file_path
        user_data_store[userId]['chunk_size'] = chunkSize
        user_data_store[userId]['chunk_overlap'] = chunkOverlap

        print(f"[upload_resume] User '{userId}'의 파일 경로 '{file_path}' 저장 완료.")
        ...
        print(f"[upload_resume] 파일 처리 중 ValueError: {str(ve)}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(ve))
    except Exception as e:
        print(f"[upload_resume] 파일 처리 중 일반 오류: {str(e)}")
...
async def chat_with_ai(request_data: ChatRequest):
    ...
    print(f"[chat_with_ai] 챗 요청 수신. User ID: {user_id}, 메시지: '{user_message}'")
    ...
    if not user_data or 'file_path' not in user_data:
        print(f"[chat_with_ai] User '{user_id}'에 대한 파일 경로가 없습니다. 파일 업로드 요청 메시지 반환.")
        # 파일 경로가 없으면 업로드 요청
        return JSONResponse(
            status_code=status.HTTP_200_OK, # 200 OK로 반환하여 프론트엔드에서 메시지 처리
            content={"aiResponse": "어떤 파일로 작업을 진행할까요? 이력서 파일을 먼저 업로드해주세요."}
        )
    
    file_path = user_data['file_path']
    chat_history = user_data.get('chat_history', [])
    print(f"[chat_with_ai] User '{user_id}'의 현재 파일 경로: {file_path}")
    print(f"[chat_with_ai] 현재 대화 기록 길이: {len(chat_history)}")
    ...
    try:
        # AgentExecutor는 동기 함수이므로 asyncio.to_thread를 사용하여 별도 스레드에서 실행
        print(f"[chat_with_ai] AgentExecutor.invoke 호출 중...")
        result = await asyncio.to_thread(agent_executor.invoke, {
            "input": user_message,
            "chat_history": chat_history,
            "current_resume_path": file_path # 현재 파일 경로를 에이전트에 전달
        })
        print(f"[chat_with_ai] AgentExecutor.invoke 호출 완료. 결과: {result}")
        ...
        print(f"[chat_with_ai] 대화 기록 업데이트 완료. 새 기록 길이: {len(chat_history)}")
        ...
        print(f"[chat_with_ai] 챗봇 처리 중 오류 발생: {e}")
